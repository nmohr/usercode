...
class DoubleStackMaker:
    ...
    def doPlot(self):
        ...
        if 'Zee' in self.datanames and 'Zmm' in self.datanames:
            ...
            addFlag = 'pp #rightarrow VZ; Z #rightarrow b#bar{b}'
        ...
        allStack.GetYaxis().SetTitleSize(ROOT.gStyle.GetTitleSize()*1.7)
        allStack.GetYaxis().SetLabelSize(ROOT.gStyle.GetLabelSize() * 1.7)
        allStack.GetYaxis().SetTitleOffset(0.82)
        theErrorGraph = ROOT.TGraphErrors(allMC)
        theErrorGraph.SetFillColor(ROOT.kGray+3)
        theErrorGraph.SetFillStyle(3013)
        theErrorGraph.Draw('SAME2')
        l.AddEntry(theErrorGraph,"Total uncert.","fl")
        #l.AddEntry(theErrorGraph,"Background uncert.","fl")
        Ymax = max(allStack.GetMaximum(),d1.GetMaximum())*1.7
        if self.log:
            allStack.SetMinimum(10)
            #Ymax = Ymax*ROOT.TMath.Power(10,1.2*(ROOT.TMath.Log(1.2*(Ymax/0.1))/ROOT.TMath.Log(10)))*(0.2*0.1)
            Ymax = Ymax*ROOT.TMath.Power(10,0.8*(ROOT.TMath.Log(0.8*(Ymax/0.1))/ROOT.TMath.Log(10)))*(0.1*0.1)
            ROOT.gPad.SetLogy()
        ...
        #tLumi = self.myText("#sqrt{s} =  %s, L = %.1f fb^{-1}"%(self.anaTag,(float(self.lumi)/1000.)),0.17,0.77,1.2)
        #tAddFlag = self.myText(addFlag,0.17,0.69,1.2)
        tA = self.myText("(a)",0.88,0.85,1.2)
        ...
        ratio.GetYaxis().SetTitleSize(ROOT.gStyle.GetTitleSize()*2.6)
        ratio.GetYaxis().SetLabelSize(ROOT.gStyle.GetLabelSize() * 2.6)
        ratio.GetYaxis().SetTitleOffset(0.43)
        ratio.Draw("E1SAME")
        ratio.SetTitle("")
        ratio.SetYTitle("#frac{Data}{MC(S+B)}")
        if not self.blind:
            tKsChi = self.myText("#chi^{2}_{ }#lower[0.1]{/^{}#it{dof} = %.2f}"%(chiScore),0.17,0.9,1.9)
        tC = self.myText("(c)",0.88,0.87,2.1)
        ...
        ratio1, error1 = getRatio(d1,bkgMC,self.xMin,self.xMax,"",self.maxRatioUncert,False)
        ...
        ratio1.GetYaxis().SetTitleSize(ROOT.gStyle.GetTitleSize()*3.51)
        ratio1.GetYaxis().SetTitleOffset(0.32)
        #ratio1.GetYaxis().SetLabelSize(ROOT.gStyle.GetLabelSize() * 3.7)
        ratio1.GetYaxis().SetLabelSize(ROOT.gStyle.GetLabelSize() * 3.51)

        m_one_line.Draw("Same")

        if not self.blind:
            tKsChi1 = self.myText("#chi^{2}_{ }#lower[0.1]{/^{}#it{dof} = %.2f}"%(chiScore1),0.17,0.865,2.57)
        tB = self.myText("(b)",0.88,0.87,2.7)
        t0 = ROOT.TText()
