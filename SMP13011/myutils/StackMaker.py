...
class StackMaker:
    def __init__(self, config, var,region,SignalRegion,setup=None):
        ...
        if 'TTbar' in self.region:
            ...
            self.addFlag2 = 'pp #rightarrow VZ; Z #rightarrow b#bar{b}'
        print self.setup
    ...
    def doSubPlot(self,signal):
        ...
        TdrStyles.tdrStyle()
        ROOT.gStyle.SetTextSize(0.78*ROOT.gStyle.GetTextSize())
        ROOT.gStyle.SetTitleSize(0.8*ROOT.gStyle.GetTitleSize())
        ROOT.gStyle.SetTitleXSize(0.85*ROOT.gStyle.GetTitleXSize())
        ROOT.gStyle.SetTitleYSize(0.8*ROOT.gStyle.GetTitleYSize())
        ROOT.gStyle.SetLabelSize(0.8*ROOT.gStyle.GetLabelSize())
        ROOT.gStyle.SetLabelSize(0.8*ROOT.gStyle.GetLabelSize('Y'),'Y')
        ROOT.gStyle.SetTitleOffset(1.2*ROOT.gStyle.GetTitleOffset())
        histo_dict = HistoMaker.orderandadd([{self.typs[i]:self.histos[i]} for i in range(len(self.histos))],self.setup)
        #sort
        print histo_dict
        sig_histos=[]
...
#        l.AddEntry(sub_d1,datatitle,'P')
        ...
        l.AddEntry(theMCOutline,"Tot. MC uncert.","fl")
        
        theErrorGraph = ROOT.TGraphErrors(sigStack.GetStack().Last().Clone())
        theErrorGraph.SetFillColor(ROOT.kGray+3)
        theErrorGraph.SetFillStyle(3013)
        theErrorGraph.Draw('SAME2')
        l.AddEntry(theErrorGraph,"Uncert. on VZ + VH","fl")
        ...
        tPrel = self.myText("CMS",0.17,0.91,1.04)
        tLumi = self.myText("#sqrt{s} =  %s, L = %.1f fb^{-1}"%(self.anaTag,(float(self.lumi)/1000.)),0.17,0.87)
        tAddFlag = self.myText(addFlag,0.17,0.83)
        bLabel = self.myText("(b)",0.8,0.6,1.1)
