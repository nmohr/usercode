...
from myutils import BetterConfigParser, StackMaker, DoubleStackMaker, HistoMaker
...
def log_s_over_b(fileList):
    ...
    stack = DoubleStackMaker(config,'logSB','plot1',False)
    stack.setup = ['VVHF','BKG']
    stack.typs = ['VVHF','BKG']
    stack.lumi = 18940.
    ...
    stack.options['pdfName'] = 'BDT_Summary.pdf'
    stack.doPlot()
...
def plot(fileList):
    ...
    for histo in histosL:
        ...
        if 'data' in histo.GetName():
            datasL.append(histo)
        if 'VH' in histo.GetName() or histo.GetName() == 'VVHF':
    ...
    overlay_histo_dict = HistoMaker.orderandadd([{overlay_typs[i]:overlayL[i]} for i in range(len(overlayL))],['VVHF','VH'])
...
#    stack.options['pdfName'] = stack.options['pdfName'].replace('.pdf','_combined_postFit_s_over_b_Hpt_weight_1.pdf'
    ...
    if mjj_sub == False:
            stack.options['pdfName'] = 'MJJ_combined.pdf'
            stack.doPlot()
    elif mjj_sub == True:
            stack.options['pdfName'] = 'MJJ_subtracted.pdf'
            stack.doSubPlot(['VVHF','VVLF','VH'])
    print 'i am done!\n'
...
##########################
fileList = []
fileList += [ 'data/MJJ/MJJ_Zll_ZmmMedPt_PostFit_s.root',
                 'data/MJJ/MJJ_Zll_ZeeMedPt_PostFit_s.root',
                 'data/MJJ/MJJ_Znn_MedPt_ZnunuMedPt_8TeV_PostFit_s.root',
                 'data/MJJ/MJJ_Wln_ch2_Wmunu2_PostFit_s.root']
    #LowPt    
fileList += [ 'data/MJJ/MJJ_Znn_LowPt_ZnunuLowPt_8TeV_PostFit_s.root',
                  'data/MJJ/MJJ_Wln_ch1_Wenu_PostFit_s.root',
                  'data/MJJ/MJJ_Wln_ch2_Wmunu_PostFit_s.root']
    #highPt    
fileList += [ 'data/MJJ/MJJ_Wln_ch1_WenuHighPt_PostFit_s.root',
                  'data/MJJ/MJJ_Zll_ZeeHighPt_PostFit_s.root',
                  'data/MJJ/MJJ_Znn_HighPt_ZnunuHighPt_8TeV_PostFit_s.root',
                  'data/MJJ/MJJ_Wln_ch2_WmunuHighPt_PostFit_s.root',
                  'data/MJJ/MJJ_Zll_ZmmHighPt_PostFit_s.root']
        
bdt_fileList = []
bdt_fileList+=[
'data/BDTVZ/BDT_Wln_VV_ch1_Wenu2_PostFit_s.root',
'data/BDTVZ/BDT_Wln_VV_ch1_Wenu3_PostFit_s.root',
'data/BDTVZ/BDT_Wln_VV_ch1_Wenu_PostFit_s.root',
'data/BDTVZ/BDT_Wln_VV_ch2_Wmunu2_PostFit_s.root',
'data/BDTVZ/BDT_Wln_VV_ch2_Wmunu3_PostFit_s.root',
'data/BDTVZ/BDT_Wln_VV_ch2_Wmunu_PostFit_s.root',
'data/BDTVZ/BDT_Zll_VV_ZeeHighPt_PostFit_s.root',
'data/BDTVZ/BDT_Zll_VV_ZmmHighPt_PostFit_s.root',
'data/BDTVZ/BDT_Znn_VV_HighPt_ZnunuHighPt_PostFit_s.root',
'data/BDTVZ/BDT_Znn_VV_LowPt_ZnunuLowPt_PostFit_s.root',
'data/BDTVZ/BDT_Znn_VV_MedPt_ZnunuMedPt_PostFit_s.root'
]
